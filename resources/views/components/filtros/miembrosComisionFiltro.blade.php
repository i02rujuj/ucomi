...
@section ('filtro')
    <form method="GET" action="{{ route('miembrosComision') }}">
        <div class="flex flex-wrap md:flex-wrap lg:flex-nowrap w-full gap-2">
            <div class="mt-2 bg-white px-6 py-4 rounded-lg shadow-md w-full">
                ...
                <div class="left-part truncate mb-2">                              
                    <select class="w-full text-sm text-gray-600 border bg-blue-50 rounded-md px-2 py-1 outline-none" id="filtroJunta" name="filtroJunta">
                        <option value="">Filtrar por junta</option>
                        @foreach ($juntas as $junta)
                            <option value="{{ $junta['id'] }}" {{ app('request')->input('filtroJunta') == $junta['id'] ? "selected":"" }}>{{ $junta->fechaConstitucion }} | {{ $junta->centro->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="left-part truncate mb-2">                              
                    <select class="w-full text-sm text-gray-600 border bg-blue-50 rounded-md px-2 py-1 outline-none" id="filtroRepresentacion" name="filtroRepresentacion">
                        ...
                        @foreach ($representacionesGeneral as $representacion)
